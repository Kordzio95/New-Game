...
def rozpocznij_blok():
    global blok_aktywny, blok_timer, blok_cooldown

    if blok_cooldown > 0 or czy_unik or pokaz_dialog or pokaz_wybor:
        return
...
def rysuj_ministatus():
    panel = pygame.Rect(605, 15, 180, 170)
    pygame.draw.rect(okno, jasny_szary, panel)
    pygame.draw.rect(okno, czarny, panel, 3)
    ...
    tekst6 = font_maly.render("CTRL blok", True, niebieski)
    tekst7 = font_maly.render("SHIFT unik", True, blekit)

    okno.blit(tekst1, (617, 28))
    okno.blit(tekst2, (617, 52))
    okno.blit(tekst3, (617, 76))
    okno.blit(tekst4, (617, 100))
    okno.blit(tekst5, (617, 124))
    okno.blit(tekst6, (617, 148))
    okno.blit(tekst7, (617, 170))
...
def rysuj_podpowiedz():
    ...
    if lokacja == "zewnatrz":
        if czy_gracz_jest_blisko_jakuba() or czy_gracz_jest_blisko_michala():
            ...
            tekst_podpowiedzi = "SPACJA atak | CTRL blok pomarancz | SHIFT unik czerwony"
...
while dziala:
    ...
    rysuj_zadanie()
    rysuj_ministatus()
    rysuj_pasek_hp(605, 195, 180, 18, gracz_hp, gracz_max_hp)
    rysuj_podpowiedz()
